...
export default function Header(children) {
  ...
  const dataMenu = [
    {
      ...
      action: async () => {
        ...
        if (infoUser && infoUser.id_user) {
          ...
          console.error('User info not found for logout');
        }
      },
    },
  ];
  ...
  React.useEffect(() => {
    // get the username from the document.location
  ...
  return (
    <div className='header'>
      ...
      <div className='header-title corinthia-bold'>Ink&nbsp; Flow</div>

      <div className='header-icons'>
        <div className='types-note-dropdown'>
          <select
            className='types-note-select'
            value={children.type}
            onChange={(e) => children.setType(e.target.value)}
          >
            {children.allTypes.map((value) => (
              <option className='types-note-option' key={value} value={value}>
                {' '}
                {value}{' '}
              </option>
            ))}
